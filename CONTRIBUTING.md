...
- **Node 8 LTS:** `generator-serverless-nodejs-ts` has a minimum Node version
...
``` bash
git clone https://github.com/EndemolShineGroup/generator-serverless-nodejs-ts.git
cd generator-serverless-nodejs-ts
yarn
yarn test
```
...
We want to ensure that `generator-serverless-nodejs-ts` is secure for everyone. If
